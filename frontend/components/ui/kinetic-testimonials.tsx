...
const KineticTestimonial: React.FC<KineticTestimonialProps> = ({
  ...
  const renderColumn = useCallback(
    (
      ...
      return (
        ...
        >
          <div
            className={`flex flex-col transform-gpu will-change-transform ${moveUp ? 'animate-scroll-up' : 'animate-scroll-down'
              }`}
            style={{
  ...
  return (
    <section
      className={`py-6 bg-transparent transition-colors duration-300 ${className}`}
    >
      <div className={`relative w-full text-zinc-900 dark:text-zinc-100 ${title || subtitle ? 'py-8 md:py-20' : 'py-0'} flex flex-col items-center overflow-hidden px-4 md:px-6`}>
        {title && (
        ...
        {subtitle && (
          <p className='text-zinc-600 dark:text-zinc-400 mb-8 md:mb-12 text-center w-full max-w-2xl px-4 text-sm'>
            {subtitle}
          </p>
        )}
